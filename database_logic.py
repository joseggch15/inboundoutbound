# Basado y extendido a partir del modulo original. Referencia: :contentReference[oaicite:0]{index=0}
import sqlite3
import json
import logging
import socket
...
from db_connection import get_connection, get_cursor, ensure_cursor, set_db_file
from constants import SYSTEM_SHIFT_DEFAULTS, SESSION_TIMEOUT_SECONDS, REPORT_DEFAULTS, Source
...
def setup_database():
    ...
    with get_connection() as conn:
        ...
        # 1b. Migracion: columna display_order para orden visual persistente
        try:
        ...
        # Migracion automatica de roles existentes en usuarios
        try:
            cursor.execute(
        ...
        # 4. Tabla Locations (Corregida y consolidada aqui)
        cursor.execute("""
        ...
        # Migracion de esquema antiguo de location
        try:
        ...
        # Migracion: columna para turnos laborales que no generan transporte ni estadia
        try: cursor.execute("ALTER TABLE shift_types ADD COLUMN no_transport INTEGER DEFAULT 0")
        except sqlite3.OperationalError: pass
        # Migracion: columna para turnos con logica 1+D (salida = end_date + 1)
        ...
        # 12. Credentials table (hashed passwords)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS credentials (
                ...
                can_manage_shift_types INTEGER DEFAULT 0
            )
        """)
...
def _hash_password(password: str) -> str:
    """Hash a password using SHA-256 with a salt. For production, use bcrypt/argon2."""
    import hashlib
    import os
    salt = os.urandom(16)
...
def _seed_default_credentials() -> None:
    """Insert default users if credentials table is empty (first run migration)."""
    with get_cursor() as cur:
        ...
        defaults = [
            ("javierteheran", "123", "RGM", "PlanStaffRGM.xlsx", True),
            ("miguelvenegas", "456", "Newmont", "PlanStaffNewmont.xlsx", True),
            ("tonyrios", "357", "Newmont", "PlanStaffNewmont.xlsx", True),
            ("admin", "123456789", "Administrator", "", True),
        ]
        for username, password, role, excel_file, can_manage in defaults:
            cur.execute(
                "INSERT OR IGNORE INTO credentials "
                "(username, password_hash, role, excel_file, can_manage_shift_types) "
                "VALUES (?, ?, ?, ?, ?)",
                (username, _hash_password(password), role, excel_file, 1 if can_manage else 0),
            )
...
def authenticate_user(username: str, password: str) -> Optional[Dict]:
    """
    Authenticate a user against the credentials table.
    Returns user info dict on success, None on failure.
    """
    with get_cursor(row_factory=True) as cur:
        cur.execute(
            "SELECT username, password_hash, role, excel_file, can_manage_shift_types "
        ...
        if row and _verify_password(password, row["password_hash"]):
            return {
                ...
                "can_manage_shift_types": bool(row["can_manage_shift_types"]),
            }
        return None


def create_credential(username: str, password: str, role: str,
...
def update_credential_password(username: str, new_password: str) -> Tuple[bool, str]:
    ...
    with get_cursor() as cur:
        cur.execute(
            ...
            (_hash_password(new_password), username.lower()),
        )
        if cur.rowcount:
...
def get_all_credentials() -> List[Dict]:
    """Get all credentials (without password hashes) for admin management."""
    with get_cursor(row_factory=True) as cur:
        cur.execute(
            "SELECT id, username, role, excel_file, can_manage_shift_types "
            "FROM credentials ORDER BY username"
        )
        return [dict(r) for r in cur.fetchall()]


def delete_credential(username: str) -> Tuple[bool, str]:
    """Delete a user credential."""
    with get_cursor() as cur:
        cur.execute("DELETE FROM credentials WHERE username = ?", (username.lower(),))
        if cur.rowcount:
...
def get_report_settings(username: str, source: str) -> Dict:
    ...
    defaults = REPORT_DEFAULTS.get(source, REPORT_DEFAULTS.get(Source.RGM.value, {})).copy()
...
def add_users_bulk(users: list, source: str) -> int:
    ...
    with get_connection() as conn:
        ...
        try:
            ...
            logger.warning("Database error when adding users in bulk: %s", e)
...
def normalize_user_order(source: str, cursor=None) -> None:
    ...
    """
    with ensure_cursor(cursor) as (cur, should_commit):
        cur.execute(
...
# -------------------------
# Locations (CRUD) con ambito por 'source'
# -------------------------
...
def update_location(loc_id: int, pickup_location: str, source: str) -> Tuple[bool, str]:
    ...
    if not pickup_location:
        return False, "Location name cannot be empty."
    with get_cursor() as cur:
        # Solo actualiza si el registro pertenece al 'source' (seguridad por ambito)
...
# --- Variantes para Administrador (pueden cambiar 'source' o operar sin ambito) ---
...
def assign_user_location_range(badge: str, start_date: date, end_date: date,
    ...
    """Inserta una asignacion de pickup/dropoff para un rango de fechas."""
    with get_cursor() as cur:
        cur.execute(
...
def get_users_with_defaults(source: str) -> list:
    ...
    with get_cursor(row_factory=True) as cur:
        ...
        # Esto nos da el usuario Y su configuracion por defecto en una sola fila.
        query = """
            SELECT
...
def get_user_location_for_date(badge: str, d: date) -> Tuple[Optional[str], Optional[str]]:
    """Busca primero una asignacion de rango que cubra la fecha; si no existe, cae al default."""
    with get_cursor(row_factory=True) as cur:
        iso = d.isoformat()
...
# EN database_logic.py (Agregar al final o en la seccion de Operations)

def update_operation_exit_time_by_date(badge: str, target_date: date, new_exit_time: datetime.time, cursor=None):
    """
    Actualiza la hora de salida (exit_date) de la operacion que cubre 'target_date'.
    Se usa cuando se rompe un iman (Separar Viaje) para definir la hora real de salida del viaje previo.
    ...
    with ensure_cursor(cursor) as (cur, should_commit):
        try:
            ...
            # 1. Buscar la operacion activa en esa fecha
            cur.execute(
            ...
            logger.error("Error updating operation exit time: %s", e)
...
def upsert_schedule_range(
    ...
    CORREGIDO: Propagacion correcta de force_new_entry para limpiar residuos.
    """
    total = 0
    d = start_d
    ...
    try:
        while d <= end_d:
            # CORRECCION: Logica estricta para Unir/Separar
            if force_new_entry_start is None:
                # No se pidio cambio explicito -> preservar lo que haya en DB (None)
                fne = None
            elif force_new_entry_start == 0:
                # "Unir" -> limpiar forzosamente TODOS los dias del rango a 0
                fne = 0
            else:
                # "Separar" (1) -> marcar solo el primer dia como 1, limpiar el resto a 0
...
def clear_schedule_range(badge: str, start_d: date, end_d: date, source: str) -> int:
    """Elimina (limpia) estado dia-a-dia en rango."""
    with get_cursor() as cur:
        cur.execute(
...
def get_shift_type_map(source: str) -> Dict[str, Dict]:
    ...
    Crucial para que los reportes de Excel sepan distinguir dias libres y dias sin transporte.
    """
...
def update_shift_type(
    ...
    """
    Actualiza un tipo de turno. Si el codigo cambia, actualiza TODAS las asignaciones en schedules
    ...
    with get_connection() as conn:
        ...
        try:
            ...
            # Si cambio el codigo, propagar a schedules
...
def delete_shift_type(type_id: int) -> Tuple[bool, str, Optional[str], Optional[str]]:
    """
    Intenta eliminar; si esta en uso, lo impide.
    ...
    with get_cursor() as cur:
        try:
            ...
            # Regla critica: impedir eliminacion si esta asignado
            cur.execute(
            ...
            if cnt and int(cnt) > 0:
                return (
                    False,
                    f"No se puede eliminar el tipo de turno '{name}' porque esta asignado a uno o mas empleados. "
...
def get_operation_overlapping(badge: str, check_date: date, cursor: Optional[sqlite3.Cursor] = None) -> Optional[Dict]:
    """
    Busca si existe una operacion activa que cubra una fecha especifica.
    Soporta cursor externo para evitar bloqueos (database is locked).
    """
    should_close = False

    # Si no nos pasan un cursor, abrimos una conexion propia (comportamiento original)
    if cursor is None:
        conn = sqlite3.connect(DB_FILE)
        conn.row_factory = sqlite3.Row  # Importante para poder convertir a dict despues
        cursor = conn.cursor()
        should_close = True

    try:
        iso = check_date.isoformat()
        # Buscamos una operacion donde start <= date <= end
        cursor.execute(
        ...
        # Solo cerramos la conexion si NOSOTROS la abrimos.
        # Si vino de fuera (transaccion), la dejamos abierta.
        if should_close:
            cursor.connection.close()
...
def delete_operations_in_range(
    ...
    """
    Elimina cualquier operacion que este TOTAL o PARCIALMENTE contenida en el rango.
...
def ensure_system_shift_types(source: str) -> None:
    ...
    """
    defaults = SYSTEM_SHIFT_DEFAULTS.get(source, [])
    if not defaults:
    ...
    with get_connection() as conn:
        ...
        try:
            ...
            logger.error("[ensure_system_shift_types] Error for source=%s: %s", source, e)
            raise
